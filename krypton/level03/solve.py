...
def matches_pattern(cipher_bigram: str, english_bigram: str, key_mapping: dict[str, str]) -> bool:
    ...
    for cipher_ch, english_ch in zip(cipher_bigram, english_bigram, strict=False):
...
def main() -> None:
    ...
    # Trigrams
    for cipher_tg, english_tg in zip(cipher_trigrams, ENGLISH_TRIGRAMS, strict=False):
        key_mapping.update(zip(cipher_tg, english_tg, strict=False))
    ...
    for c, e in zip(cipher_freq_order, ENGLISH_FREQ, strict=False):
